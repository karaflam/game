...
> **Note de révision (2026-08-03)** — Ce spec décrit encore l'approche initialement retenue pour les mains : géométrie procédurale low-poly (boîtes/cylindres) avec un auto-rig d'os personnalisé pliant chaque doigt selon un angle de courbure. Cette approche a bien été construite, mais la revue visuelle par le développeur humain a révélé quatre défauts successifs : géométrie trop anguleuse/boxy, puis (après passage à un maillage réaliste riggé) direction de flexion des doigts incorrecte, puis mauvaise correspondance doigt/anatomie, puis fermeture du poing insuffisante. Le développeur a finalement fourni deux modèles 3D pré-posés (générés avec Meshy AI : un poing serré et un signe de paix) et a demandé de basculer sur ceux-ci plutôt que de poursuivre le rig. L'implémentation livrée rend donc 3 maillages pré-posés distincts (main ouverte pour feuille/neutral, les deux modèles Meshy pour pierre/ciseau) et fait un crossfade d'opacité entre eux aux transitions de pose — il n'y a ni rig d'os ni données de courbure par doigt. Le reste de ce spec (fondations, thèmes, qualité adaptative, etc.) reste exact et n'a pas été mis à jour ci-dessous.
